<template>
  <div id="app">
    <div class="header">
      ...
      <div class="navigator">
        <ul class="nav-list">
          <li v-for="(item, index) in tabList" :key="index" @click="switchTab(index)" :class="{active: currentPage === index}">
            {{ item.txt }}
          </li>
        </ul>
      ...
      >
        <cube-slide-item v-for="(item, index) in tabList" :key="index">
          <div class="match-list-wrapper">
            <!--<match-list :source="source" ></match-list>-->
            <match-list :type="item.type" :source="source"></match-list>
          </div>
        </cube-slide-item>
        <div slot="dots"></div>
      </cube-slide>
